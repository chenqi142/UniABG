...
def create_contrastive_pairs_with_confidence_pytorch_json(
    uav_features_tensor: torch.Tensor,    # Shape: [B_uav, feature_dim]
    uav_labels_tensor: torch.Tensor,      # Shape: [B_uav] (numpy)
    uav_image_paths_list: List[str],      # Length: B_uav

    satellite_features_tensor: torch.Tensor, # Shape: [B_sat, feature_dim]
    satellite_image_paths_list: List[str], # Length: B_sat

    output_json_path: Optional[str] = None, # Path to save the output JSON file

    # Optional: Satellite cluster labels
    satellite_cluster_labels_tensor: Optional[torch.Tensor] = None # Shape: [B_sat]
) -> List[Tuple[int, str, str, float]]: # Returns list of (label, uav_path, sat_path, confidence_score)
    """
    根据描述的规则创建无人机-卫星图片对，并计算每个类别配对的置信度。
    置信度定义为：某UAV类别内所有UAV特征与该类别最终选定的卫星图片特征之间的平均余弦相似度。

    核心规则：对每个UAV类别，找出其成员最常匹配到的那张特定卫星图片。

    参数:
    - uav_features_tensor: torch.Tensor, 无人机特征张量。
    - uav_labels_tensor: torch.Tensor, 无人机图片对应的伪标签。
    - uav_image_paths_list: list, 无人机图片路径列表。
    - satellite_features_tensor: torch.Tensor, 所有卫星图片的特征张量。
    - satellite_image_paths_list: list, 所有卫星图片路径列表。
    - output_json_path: str (可选), 保存图片对路径和置信度的JSON文件路径。
    - satellite_cluster_labels_tensor: torch.Tensor (可选), 每张卫星图片对应的聚类伪标签。

    返回:
    - contrastive_pairs_with_confidence: list of tuples. 
                                       [(uav_path, sat_path, confidence_score), ...]。
                                       如果路径未提供或无法配对，列表可能为空。
    """

    # --- Input Validation and Conversion ---
    if not (isinstance(uav_features_tensor, torch.Tensor) and
            isinstance(satellite_features_tensor, torch.Tensor)):
        print("错误：特征输入应为 PyTorch 张量。")
        return []


    if not uav_image_paths_list or not satellite_image_paths_list:
        print("错误：进行图片对路径保存时，无人机和卫星的图片路径列表不能为空。")
        return []

    # pdb.set_trace() # Removed debugger breakpoint

    num_uav_images = uav_features_tensor.size(0)
    if not (uav_labels_tensor.shape[0] == num_uav_images and len(uav_image_paths_list) == num_uav_images):
        print(f"错误：无人机特征 ({num_uav_images}), "
              f"标签 ({uav_labels_tensor.shape[0]}), " # Corrected missing parenthesis
              f"和路径 ({len(uav_image_paths_list)}) 数量不一致。")
        return []

    num_satellite_images = satellite_features_tensor.size(0)
    if len(satellite_image_paths_list) != num_satellite_images:
        print(f"错误：卫星特征 ({num_satellite_images}) 和路径 ({len(satellite_image_paths_list)}) 数量不一致。")
        return []
    if num_satellite_images == 0:
        print("错误：卫星图像列表为空，无法进行匹配。")
        return []


    if satellite_cluster_labels_tensor is not None and satellite_cluster_labels_tensor.shape[0] != num_satellite_images:
        print(f"错误：卫星聚类标签 ({satellite_cluster_labels_tensor.shape[0]}) "
              f"和卫星特征 ({num_satellite_images}) 数量不一致。")
        return []
    
    num_uav = uav_features_tensor.shape[0]
    num_sat = satellite_features_tensor.shape[0]
    device = uav_features_tensor.device
    satellite_features_tensor = satellite_features_tensor.to(device)


   

    similarity_matrix = torch.matmul(uav_features_tensor, satellite_features_tensor.transpose(0, 1))
    
    # best_satellite_indices 形状: [B_uav], 值为匹配到的卫星图片在 satellite_features_tensor 中的索引
    best_satellite_similarities, best_satellite_indices = torch.max(similarity_matrix, dim=1)

     # --- 构建配对 ---
    contrastive_pairs = []
    for uav_idx in range(num_uav):
        similarity_score = best_satellite_similarities[uav_idx].item()
        sat_idx = best_satellite_indices[uav_idx].item()
        
        # 确保索引有效
        if 0 <= sat_idx < num_sat:
            uav_path = uav_image_paths_list[uav_idx]
            sat_path = satellite_image_paths_list[sat_idx]
            contrastive_pairs.append((sat_idx, uav_path, sat_path, similarity_score))
    ...
    return contrastive_pairs, uav_to_satellite_map
...
##############################
            test_batch=64
            query_folder_test='/data0/chenqi_data/University-Release/test/query_drone'
            gallery_folder_test='/data0/chenqi_data/University-Release/test/gallery_satellite'
...
def main_worker_stage2_inter_view(args):
    ...
    for epoch in range(args.epochs):
        ...
        # results = create_contrastive_pairs_with_confidence_pytorch_json(
        #     uav_features_tensor=features_drone1, #使用修改后的特征以获得可预测结果
        #     uav_labels_tensor=pseudo_labels_drone,
        #     uav_image_paths_list=uav_image_paths_list_main,
        #     satellite_features_tensor=features_satellite, #使用修改后的特征以获得可预测结果
        #     satellite_image_paths_list=satellite_image_paths_list_main,
        #     output_json_path=output_file,
        #     satellite_cluster_labels_tensor=labels_satellite
        # )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu_ids', default=(4, 5, 6, 7), type=tuple)
    ...
    parser.add_argument('--device', default='cuda:4' if torch.cuda.is_available() else 'cpu', type=str)
    ...
    parser.add_argument('--logs-dir', type=str, metavar='PATH', default="/data1/chenqi/DAC/check_point")
